...
*"  EXPORTING
*"     VALUE(COUNT) TYPE  I
*"     REFERENCE(WASINGLE) LIKE  ZEM_MBPADDRSMTP STRUCTURE
*"        ZEM_MBPADDRSMTP
*"  TABLES
*"      SELECTTABLE STRUCTURE  ZEM_MBPADDRSMTP OPTIONAL
*"      LIMITTABLE OPTIONAL
*"      WHERE OPTIONAL
*"  EXCEPTIONS
*"      BLOCKED_PARTNER
*"----------------------------------------------------------------------
  ...
  DATA: lt_limit_list      TYPE limit_tt_list,
        lt_mbpaddrsmtp     TYPE TABLE OF zem_mbpaddrsmtp,
  ...
  FIELD-SYMBOLS: <lv_partner> TYPE any,
                 ...
                 <ls_but000>  TYPE zem_mbpaddrsmtp.
  ...
  IF single IS NOT INITIAL.
    ...
    SELECT SINGLE * FROM zem_mbpaddrsmtp
  ...
  IF NOT selecttable IS REQUESTED.
    ...
    IF NOT lt_limit_list[] IS INITIAL.
      ...
      SELECT COUNT(*) FROM zem_mbpaddrsmtp
             INTO @count
...
*    SELECT requested data records
    IF NOT lt_limit_list[] IS INITIAL.
      ...
      SELECT * FROM zem_mbpaddrsmtp
               INTO CORRESPONDING FIELDS OF TABLE @lt_mbpaddrsmtp
               FOR ALL ENTRIES IN @lt_limit_list
               WHERE partner = @lt_limit_list-partner AND
                    (where).
    ELSE.
    ...
    REFRESH: selecttable.
    SORT lt_mbpaddrsmtp BY partner.
    ...
    selecttable[] = CORRESPONDING #( lt_mbpaddrsmtp ).
