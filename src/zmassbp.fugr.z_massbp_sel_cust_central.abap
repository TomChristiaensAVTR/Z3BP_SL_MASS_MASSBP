FUNCTION z_massbp_sel_cust_central.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"  EXPORTING
*"     VALUE(COUNT) TYPE  I
*"     REFERENCE(WASINGLE) LIKE  ZEM_MBPCUSTKNA1 STRUCTURE
*"        ZEM_MBPCUSTKNA1
*"  TABLES
*"      SELECTTABLE STRUCTURE  ZEM_MBPCUSTKNA1 OPTIONAL
*"      LIMITTABLE OPTIONAL
*"      WHERE OPTIONAL
*"  EXCEPTIONS
*"      BLOCKED_PARTNER
...
*Internal tables
  DATA: lt_limit_list      TYPE limit_tt_list,
        lt_mass_bp_kna1    TYPE TABLE OF zim_mass_bp_kna1,
...
*Fieldsymbols
  FIELD-SYMBOLS: <lv_partner> TYPE any,
                 ...
                 <ls_but000>  TYPE zim_mass_bp_kna1.
  ...
  IF single IS NOT INITIAL.
    ...
    SELECT SINGLE * FROM zim_mass_bp_kna1
  ...
  IF NOT selecttable IS REQUESTED.
    ...
    IF NOT lt_limit_list[] IS INITIAL.
      ...
      SELECT COUNT(*) FROM zim_mass_bp_kna1
             INTO @count
...
*    SELECT requested data records
    IF NOT lt_limit_list[] IS INITIAL.
      ...
      SELECT * FROM zim_mass_bp_kna1
               INTO CORRESPONDING FIELDS OF TABLE @lt_mass_bp_kna1
               FOR ALL ENTRIES IN @lt_limit_list
               WHERE partner = @lt_limit_list-partner AND
                    (where).
    ELSE.
    ...
    REFRESH: selecttable.
    SORT lt_mass_bp_kna1 BY partner
                      valid_from
                      valid_to.
    DELETE ADJACENT DUPLICATES
           FROM lt_mass_bp_kna1
           COMPARING partner
                     valid_from
                     valid_to.
    ...
    LOOP AT lt_mass_bp_kna1
      ...
      ls_master_data_in-customers = VALUE #( BASE ls_master_data_in-customers ( header-object_instance-kunnr = <ls_but000>-partner
                                                                                header-object_task = zif_massbp_co=>gc_objtask_modify ) ).

      ls_partner_temp = <ls_but000>-partner.
    ...
    IF ls_master_data_in-customers IS NOT INITIAL.
      ...
      LOOP AT ls_master_data_out-customers
          ...
          WHERE central_data-central-data-cvp_xblck <> 'X'.
        ...
        <ls_selecttable_ins> = CORRESPONDING #( BASE ( <ls_selecttable_ins> ) <ls_customer_data>-central_data-central-data ).
      ENDLOOP.
    ENDIF.
