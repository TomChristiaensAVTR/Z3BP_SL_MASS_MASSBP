FUNCTION z_massbp_sel_cust_sales.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"  EXPORTING
*"     VALUE(COUNT) TYPE  I
*"     REFERENCE(WASINGLE) LIKE  ZEM_MBPCUSTKNVV STRUCTURE
*"        ZEM_MBPCUSTKNVV
*"  TABLES
*"      SELECTTABLE STRUCTURE  ZEM_MBPCUSTKNVV OPTIONAL
*"      LIMITTABLE OPTIONAL
*"      WHERE OPTIONAL
*"  EXCEPTIONS
*"      BLOCKED_PARTNER
...
*Internal tables
  DATA: lt_limit_list      TYPE limit_tt_list,
        lt_mass_bp_knvv    TYPE TABLE OF zim_mass_bp_knvv,
...
*Fieldsymbols
  FIELD-SYMBOLS: <lv_partner> TYPE any,
                 ...
                 <ls_but000>  TYPE zim_mass_bp_knvv.
  ...
  IF single IS NOT INITIAL.
    ...
    SELECT SINGLE * FROM zim_mass_bp_knvv
  ...
  IF NOT selecttable IS REQUESTED.
    ...
    IF NOT lt_limit_list[] IS INITIAL.
      ...
      SELECT COUNT(*) FROM zim_mass_bp_knvv
             INTO @count
...
*    SELECT requested data records
    IF NOT lt_limit_list[] IS INITIAL.
      ...
      SELECT * FROM zim_mass_bp_knvv
               INTO CORRESPONDING FIELDS OF TABLE @lt_mass_bp_knvv
               FOR ALL ENTRIES IN @lt_limit_list
               WHERE partner = @lt_limit_list-partner AND
                    (where).
    ELSE.
    ...
    REFRESH: selecttable.
    SORT lt_mass_bp_knvv BY partner
                      valid_from
                      valid_to.
    DELETE ADJACENT DUPLICATES
           FROM lt_mass_bp_knvv
           COMPARING partner
                     valid_from
                     valid_to.
